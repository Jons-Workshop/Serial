...
char *	Serial::test_for_rx_message	()	{	//	Read in any received chars into linear command line buffer
	while	(!(rx_buff_empty))	{
		*ch = (char)rxbuff[rx_offptr++];	//	Get next received characters, if any have been received.
...
//		set_error (SerialErrors::SOME_OTHER);	//	Proved no problem here
	...
	tx_buff_empty = false;
//	tx_any_buffered	()	;	//	Perhaps better to leave this to be done regularly from forever loop
	return	(true);
}
...
void	Serial::rx_intrpt_handler_core_DMA	()	{	//	UART has rec'd a single char, DMA has already placed it on circular buffer.
	...
	newRx.put(rxbuff[0]);	//	New 2025 CircularBuffer
	start_rx	()	;
/*
//	if	(++rx_onptr >= RXBUFF_SIZE)
//		rx_onptr = 0;			//	Circular buffer offset now updated
	++rx_onptr %= RXBUFF_SIZE;
//	if	(HAL_OK != HAL_UART_Receive_DMA(m_huartn, rxbuff + rx_onptr, 1))	//	Set DMA looking for next rx byte to come along
	if	(HAL_OK != HAL_UART_Receive_DMA(m_huartn, rxbuff, 1))	//	Set DMA looking for next rx byte to come along
		set_error	(SerialErrors::HAL_RX_DMA);	//	HAL was not OK (Dec2024)
	rx_buff_full 	= (rx_onptr == rx_offptr);
	rx_buff_empty	= false;						//	Because at least one char has been received into rxbuff
*/
